...
const christmasNames = () => {
  ...
  const drawName = (chooser) => {
    randomNumber =
      Math.floor(Math.random() * (namesInHat.length - 1 - 0 + 1)) + 0;
  ...
  while (namesInHat.length) {
    ...
    if (
      (!namesInHat.length && pickedName == chooser) ||
      (!namesInHat.length && pickedName == restricted[chooser])
    ) {
      console.log(
        "The last person to draw drew themselves or their significant other! Starting over..."
      );
      restart();
      continue;
    }
    ...
    while (pickedName == restricted[chooser]) {
      console.log(
        `${chooser} picked ${pickedName}, their significant other. Putting name back in hat.`
      );
      putNameBack(pickedName);
      pickedName = drawName(chooser);
    }
